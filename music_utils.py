...
def data_processing(corpus, values_indices, m = 60, Tx = 30):
    """
    Processes a corpus into input-output pairs for sequence learning.

    This function transforms a linear sequence (corpus) into a set of training examples
    for a sequence-to-sequence model. It generates semi-redundant sequences by randomly
    sampling segments from the corpus and encoding them as one-hot vectors.

    Args:
        corpus (list): The corpus of sequential values (e.g., notes, characters) to process.
        values_indices (dict): A mapping from corpus values to their corresponding indices.
        m (int, optional): The number of training examples to generate. Defaults to 60.
        Tx (int, optional): The length of each sequence. Defaults to 30.

    Returns:
        tuple:
            - X (numpy.ndarray): Input data of shape (m, Tx, N_values) where N_values is
              the number of unique values in the corpus. One-hot encoded, with dtype=bool.
            - Y (numpy.ndarray): Target data, shifted by one timestep (predicting the next
              value in sequence). Shape is (Tx, m, N_values) with dtype=bool.
            - N_values (int): The number of unique values in the corpus.
    """
